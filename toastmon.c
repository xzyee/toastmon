...
--*/
{
    ...
    status = WdfDriverCreate(
        ...
        &config,          // 刚刚被初始化，带ToastMon_EvtDeviceAdd函数地址
        WDF_NO_HANDLE
        );
...
--*/
{
    ...
    // 创建一个framework device object.
    status = WdfDeviceCreate(&DeviceInit,//in-out，如果执行没有错误，会被设为NULL 
		                     &attributes, //带ToastMon_EvtDeviceContextCleanup函数
		                     &device);//输出，创建的设备对象，wdf牌
    if (!NT_SUCCESS(status)) {
	...
	//----------------------------------------------------------------
	// 现在要设置一下设备扩展：一个collection对象，和锁对象，都是wdf牌
	//----------------------------------------------------------------
	//
	...
	//creates a framework collection object.
    status = WdfCollectionCreate(&attributes,
                                &deviceExtension->TargetDeviceCollection);//创建的collection对象
    if (!NT_SUCCESS(status))
    {
    ...
    status = IoRegisterPlugPlayNotification (
                EventCategoryDeviceInterfaceChange, //EventCategory,枚举
                PNPNOTIFY_DEVICE_INTERFACE_INCLUDE_EXISTING_INTERFACES, //flag
                (PVOID)&GUID_DEVINTERFACE_TOASTER, //data
                WdfDriverWdmGetDriverObject(WdfDeviceGetDriver(device)),//drverobject，传统wdm牌
                (PDRIVER_NOTIFICATION_CALLBACK_ROUTINE)
                    ToastMon_PnpNotifyInterfaceChange, //回调函数，具有规定的格式
                (PVOID)deviceExtension,                //回调函数的context
                &deviceExtension->NotificationHandle);  //输出
	...
	// 注册...通知，这是WMI的
	//----------------------------------------------------------------
    RegisterForWMINotification(deviceExtension);//注册WMI通知

    return status;
	/*typedef struct _DEVICE_EXTENSION {
    WDFDEVICE           WdfDevice; //= device;
    WDFIOTARGET         ToasterTarget;
    WDFCOLLECTION       TargetDeviceCollection;//通过注册函数被设置
    WDFWAITLOCK         TargetDeviceCollectionLock;//通过WdfWaitLockCreate被设置
    PVOID               WMIDeviceArrivalNotificationObject;/通过WmiOpenBlock被设置，是data object对象
    } DEVICE_EXTENSION, *PDEVICE_EXTENSION;*/
}

//WdfDeviceCreate时要用的函数，在设备被移除前执行一切动作
#pragma warning(push)
...
--*/
{
    ...
    if(deviceExtension->NotificationHandle) {
        IoUnregisterPlugPlayNotification(deviceExtension->NotificationHandle);//反注册pnp通知
    }
    ...
    // and deleted.
    //
    deviceExtension->TargetDeviceCollection = NULL;//不需要调用delete，会自动删除

    UnregisterForWMINotification(deviceExtension);//反注册WMI回调
...
//pnp回调，针对EventCategoryDeviceInterfaceChange枚举对应的事件
...
--*/
{
	...
	USHORT Version;
	USHORT Size;
	GUID Event; //应该等于GUID_DEVICE_INTERFACE_ARRIVAL
	//
	// Event-specific data
	//
	GUID InterfaceClassGuid; //应该和...相等
	...
	*/
    ...
    if(IsEqualGUID( (LPGUID)&(devNotificationStruct->Event),
                     (LPGUID)&GUID_DEVICE_INTERFACE_ARRIVAL )) { //"Arrival！"

        KdPrint(("Arrival Notification\n"));

		//Open the I/O target and preallocate any resources
...
//在ToastMon_PnpNotifyInterfaceChange中被调用，当设备到来时
NTSTATUS
Toastmon_OpenDevice(
    WDFDEVICE Device,
...
--*/
{
	...
	//creates a remote I/O target for a specified device.
    status = WdfIoTargetCreate(deviceExtension->WdfDevice, //设备对象，句柄
                            &attributes,
                            &ioTarget);//输出，创建的对象
    if (!NT_SUCCESS(status)) {
	...
	//下面这一句成立的条件：（1）WDF_DECLARE_CONTEXT_TYPE_WITH_NAME(TARGET_DEVICE_INFO, GetTargetDeviceInfo)
	//                    （2）在上面通过WdfIoTargetCreate把iotarget和TARGET_DEVICE_INFO联系起来！！！
    ...
    WDF_IO_TARGET_OPEN_PARAMS_INIT_OPEN_BY_NAME(
        &openParams, //WDF_IO_TARGET_OPEN_PARAMS
        SymbolicLink,
        STANDARD_RIGHTS_ALL);

    openParams.ShareAccess = FILE_SHARE_WRITE | FILE_SHARE_READ;//继续初始化
    //
	...
	//opens a remote I/O target so the driver can send I/O requests to it.
	...
	//-------------------------------------------------------------------
	// 创建2个空的请求，一个为了读，一个为了写
	//-------------------------------------------------------------------
    //
    // Create two requests - one for read and one for write.
    //
    WDF_OBJECT_ATTRIBUTES_INIT(&attributes);
    attributes.ParentObject = ioTarget; //父亲

	//creates an empty framework request object.
    ...
    WDF_TIMER_CONFIG_INIT(&wdfTimerConfig,
                          Toastmon_EvtTimerPostRequests);//timer执行的回调函数
	...
	//creates a framework timer object.
    ...
    WdfTimerStart(targetDeviceInfo->TimerForPostingRequests,
                                        WDF_REL_TIMEOUT_IN_MS(1));//1ms时钟间隔
    ...
    //
    targetDeviceInfo->Opened = TRUE; //处于打开状态

    *Target = ioTarget; //输出到函数外

    return status;

}

//在Toastmon_OpenDevice中被当成回调函数填入，在IRP_MN_QUERY_REMOVE时被调用
//openParams.EvtIoTargetQueryRemove = ToastMon_EvtIoTargetQueryRemove;
NTSTATUS
...
--*/
{
	...
	// temporarily closes a specified remote I/O target because the target device might soon be removed.
...
//在Toastmon_OpenDevice中被当成回调函数填入，在IRP_MN_CANCEL_REMOVE时被调用
//openParams.EvtIoTargetRemoveCanceled = ToastMon_EvtIoTargetRemoveCanceled;
VOID
...
//在Toastmon_OpenDevice中被当成回调函数填入:
//    openParams.EvtIoTargetRemoveComplete = ToastMon_EvtIoTargetRemoveComplete; 
//在IRP_MN_REMOVE_DEVICE or IRP_MN_SURPRISE_REMOVAL时被调用
VOID
...
--*/
{
    ...
    //
    WdfObjectDelete(IoTarget);//iotarget在此被删除！

    return;

}

// 创建timer对象时，被当成回调函数填入...
// WDF_TIMER_CONFIG_INIT(&wdfTimerConfig,
//	   Toastmon_EvtTimerPostRequests);//timer执行的回调函数
VOID
...
--*/
{
   ...
   //-----------------------------------------------------
    ...
    WdfRequestSetCompletionRoutine(request,
                   Toastmon_ReadRequestCompletionRoutine, //完成函数，在后面
                   targetInfo);
	...
	targetInfo->ReadRequest = NULL; //技巧，学习
